...
import { cities } from "../components/home/data"
...
const Spa = () => {
  ...
  const word = "SPA & Hamam"

  useEffect(() => {
    // Fetch establishment types when component mounts
  ...
  const handleInputChange = (e) => {
    ...
    // Find the matching establishment type and set its ID
    const matchingType = establishmentTypes.find((type) => type.name.toLowerCase() === value.toLowerCase())

    if (matchingType) {
      setSelectedEstablishmentId(matchingType.id)
    } else {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    navigate(`/post-filter?location=${locationQuery}&establishmentId=${selectedEstablishmentId}`)
  }
  ...
  return (
    <Layout>
      ...
      <section className="relative bg-gradient-to-r from-slate-900 to-slate-800 py-16 md:py-24">
        <div className="absolute inset-0 opacity-20 bg-[url('https://images.unsplash.com/photo-1544161515-4ab6ce6db874?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2070&q=80')] bg-cover bg-center"></div>
        <div className="container mx-auto relative z-10">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <p className="text-slate-300 mb-8 text-lg">
              Trouvez votre Spa & Hamam à <span className="text-white opacity-70 inline-block">{displayedWord1}</span>
            </p>
            <form
              onSubmit={handleSubmit}
              className="w-full max-w-3xl lg:rounded-full rounded-3xl sm:bg-transparent md:bg-white p-0 md:p-2 shadow-2xl backdrop-blur-md transform transition-all duration-1000 hover:shadow-[0_20px_50px_rgba(0,35,102,0.2)]"
            >
          ...
          <div className="w-full md:w-[90%] lg:w-[90%] mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {cities.slice(0, showAllCities ? cities.length : 3).map((city, index) => (
                <a
                  key={index}
                  href={`/post-filter?location=${city.name}&establishmentId=${selectedEstablishmentId}`}
                  className="group bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300"
                >
                  <div className="relative h-56 overflow-hidden">
                    <img
                      src={city.image || "/placeholder.svg"}
                      alt={`Spa & Hamam à ${city.name}`}
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            ...
            <p className="text-slate-600 max-w-2xl mx-auto">
              ...
              rendez-vous dans un SPA ou Hamam.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="bg-slate-50 p-6 rounded-xl text-center">
              ...
              <p className="text-slate-600">Tous nos établissements sont qualifiés et vérifiés</p>
            </div>
            ...
            <div className="bg-slate-50 p-6 rounded-xl text-center">
              ...
              <p className="text-slate-600">Consultez les avis des clients pour choisir le meilleur SPA</p>
            </div>
          </div>
        </div>
      </section>
